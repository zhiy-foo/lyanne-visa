...
- [ ] 1.1 Scaffold a Next.js App Router app in TypeScript under `src/` with Tailwind CSS, ESLint and Vitest; verify `npm run build`, `npm run lint` and `npm test` all succeed on the empty app
- [ ] 1.2 Initialise the Supabase CLI project (`supabase/`), add `.env.example` with `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_SITE_URL`; verify `supabase start` runs locally and `.env.local` is git-ignored
...
- [ ] 2.4 Add `add_child`, `rename_child`, `add_guardian`, `remove_guardian`, `add_place`, `update_place`, `add_host`, `remove_host` enforcing rules 10, 11, 14, 19, 20 and the time-zone check; verify one test per refusal: host adds child, parent adds place, non-parent of the child links/unlinks, co-parent email not an active parent account (unknown / host / waiting), email in another letter case succeeds, last parent removal, last host removal, non-host edits place, unknown time zone, admin links a host as parent — plus a happy path per function
