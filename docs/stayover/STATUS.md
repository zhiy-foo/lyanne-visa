...
- After the first deployment — fine-tuning (decided 2026-09-24):
  ...
  - Check whether Gmail shows its inline event card (Yes/No/Add to calendar) for a real, non-alias recipient; `+` aliases of the app Gmail don't show it because Gmail treats the invite as self-sent. Only if the card doesn't appear: consider an "Add to Google Calendar" link as a fallback (trade-off: an event added that way won't update or cancel with the stay).
