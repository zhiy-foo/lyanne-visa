...
- **To confirm after deployment:** Vercel supports `after()` natively via `waitUntil` — verify with one real send after the first deployment. Correctness does not depend on it (the traffic-triggered fallback in `src/proxy.ts` covers the case where it does nothing), but promptness does.
